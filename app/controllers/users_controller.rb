class UsersController < ApplicationController
  ...
  def show
    ...
    @books = @user.books
    @book = Book.new
    @currentRoomUser = Contact.where(user_id: current_user.id)  #current_userが既にルームに参加しているか判断
    @receiveUser = Contact.where(user_id: @user.id)  #他の@userがルームに参加しているか判断
